...
export function getMetaForCategory(category: string): MetaEntry[] {
  const metaPath =
    category === ROOT_CATEGORY ? path.join(DOCS_DIR, '_meta.tsx') : path.join(DOCS_DIR, category, '_meta.tsx')
...
function parseMetaFile(metaPath: string): MetaEntry[] {
  ...
  const exportMatch = content.match(/export default\s*\{([\s\S]*)\}\s*satisfies/)
  ...
  const entryPattern = /['"]?([^'":,\s]+)['"]?\s*:\s*\{[^}]*?title:\s*['"]([^'"]+)['"][^}]*?\}/g
