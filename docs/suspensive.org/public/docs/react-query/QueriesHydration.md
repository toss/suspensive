---
url: /docs/react-query/QueriesHydration
---

# QueriesHydration

> **Experimental:** `<QueriesHydration/>` is an experimental feature, so this interface may change.
...
Use `queryOptions` with `QueriesHydration` to prefetch data on the server. For better HTML Streaming, wrap each component with its own `Suspense` and `QueriesHydration`:
...
You can also use `infiniteQueryOptions` with `QueriesHydration`. You can mix regular queries and infinite queries in the same array. For better HTML Streaming, wrap each component with its own `Suspense` and `QueriesHydration`:
...
An array of queries or infinite queries to be prefetched on the server. Each query must have a `queryKey`. You can mix regular queries and infinite queries in the same array.
...
type QueriesHydrationProps = {
  ...
  timeout?: number // Optional (unit: ms)
}
```
...
Limits the time allowed for fetching queries on the server. An error is thrown if query fetching exceeds this limit. When not set, no timeout is applied.

When `skipSsrOnError` is true or passed as `{ fallback: ReactNode }`, exceeding the timeout causes the server to give up rendering and the browser will attempt to render instead.
...
const Page = async ({ userId }: { userId: number }) => {
  return (
    <>
      <Suspense fallback={<div>Loading...</div>}>
        {/* An error is thrown if the query takes longer than 1000ms to fetch */}
