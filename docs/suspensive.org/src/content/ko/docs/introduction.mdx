...
React에는 Suspense와 lazy가 있고, Error Boundary를 구현할 수 있는 인터페이스를 제공합니다. 하지만 실제 애플리케이션에서 사용해 보면 빈틈이 드러납니다 — Suspense 바운더리에서 SSR을 건너뛰는 내장 방법이 없고, Error Boundary는 직접 구현하려면 보일러플레이트가 많으며, 이 모든 것을 조율할 방법이 없습니다. React뿐 아니라, 데이터 페칭 라이브러리와 상태 관리 라이브러리에도 각각의 빈틈이 있습니다.
...
direction TB
A["Suspense에서 SSR 건너뛰기 내장 API 없음"]
