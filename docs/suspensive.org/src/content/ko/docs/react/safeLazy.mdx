...
`safeLazy` 함수는 컴포넌트 로딩이 실패할 때 자동 오류 복구를 제공하는 React의 `lazy` 함수의 안전한 래퍼입니다. 컴포넌트를 로드하려고 시도하고, 로딩이 실패하면 페이지를 한 번 새로고침하여 다시 시도하여 네트워크 문제나 일시적인 로딩 실패 시 더 나은 사용자 경험을 제공합니다.

## Import

```tsx
import { safeLazy } from '@suspensive/react'
```
...
1. **첫 번째 시도**: 컴포넌트가 처음 로드에 실패할 때, `safeLazy`는:
...
const UserProfile = safeLazy(() => import('./UserProfile'))
...
const Header = safeLazy(() => import('./Header'))
const Sidebar = safeLazy(() => import('./Sidebar'))
const MainContent = safeLazy(() => import('./MainContent'))
...
다음과 같은 경우에는 `safeLazy`를 사용하지 말아야 합니다:
...
API는 `React.lazy`와 동일하므로 기존 지연 컴포넌트를 `safeLazy`로 쉽게 교체하여 자동 오류 복구를 사용할 수 있습니다.
