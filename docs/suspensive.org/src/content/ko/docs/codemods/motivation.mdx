---
description: '@suspensive/codemods가 존재하는 이유: Suspensive 메이저 버전 간 브레이킹 체인지 마이그레이션 자동화.'
---
...
Codemod는 코드베이스 내에서 반복적이고 대규모로 발생하는 작업을 자동화하여 코드를 효율적으로 업데이트할 수 있는 강력한 도구입니다. 수많은 변경 사항을 수동으로 검토할 필요 없이 빠르고 안전하게 업데이트할 수 있습니다.

Suspensive는 API 업데이트와 더 이상 사용되지 않는 API를 쉽게 관리할 수 있도록 Codemod를 제공합니다.
