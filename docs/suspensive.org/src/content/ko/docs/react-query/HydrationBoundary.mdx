...
`<HydrationBoundary/>`는 `@tanstack/react-query` v5를 사용할 때만 사용 가능합니다.
...
> 💡 **추천 읽기**: 이 컴포넌트를 사용하기 전에 [TanStack Query Advanced Server Rendering 가이드](https://tanstack.com/query/latest/docs/framework/react/guides/advanced-ssr)를 읽어보시는 것을 권장합니다. 이 가이드는 Server Components, streaming, 그리고 Next.js App Router에 대해 다룹니다. 이러한 개념을 이해하면 `HydrationBoundary`를 훨씬 더 편하게 사용할 수 있습니다.

## 기본 사용법

`HydrationBoundary`에서 `queryOptions`를 사용하여 서버에서 데이터를 미리 불러올 수 있습니다. HTML Streaming을 최대화하려면 각 컴포넌트를 별도의 `Suspense`와 `HydrationBoundary`으로 감싸는 것이 좋습니다:
...
`HydrationBoundary`에서 `infiniteQueryOptions`도 사용할 수 있습니다. 일반 쿼리와 infinite 쿼리를 같은 배열에 섞어서 사용할 수 있습니다. HTML Streaming을 최대화하려면 각 컴포넌트를 별도의 `Suspense`와 `HydrationBoundary`으로 감싸는 것이 좋습니다:
...
1. **RSC(React Server Component)**: `HydrationBoundary`에서 쿼리 실행
...
**중요**: 1단계에서 성공한 경우(에러 없음), 데이터는 이미 클라이언트에 hydrate되어 있습니다. 이 경우 브라우저의 개발자도구 네트워크 탭에서 확인할 수 있듯이 2단계와 3단계에서 추가적인 fetch 요청이 발생하지 않습니다. 데이터가 캐시에서 제공되기 때문입니다. 이것이 `HydrationBoundary`를 사용한 서버 사이드 prefetching의 효율성을 보여줍니다.
...
`<HydrationBoundary/>`를 사용하면 이 모든 과정이 자동으로 처리됩니다:
...
Dependent queries(의존적 쿼리)를 처리하면서도 각 컴포넌트를 별도의 `HydrationBoundary`으로 감싸면 독립적인 Suspense 경계로 Streaming의 이점을 최대화할 수 있습니다. 같은 `queryClient`를 공유하여 첫 번째 쿼리 결과를 두 번째 쿼리에서 사용할 수 있습니다.
...
Next.js streaming과 함께 `HydrationBoundary`를 사용하는 실제 예제입니다:
...
**팁**: 라이브 데모에서 "4. no error (Best Practice)" 케이스를 확인하고 브라우저의 개발자도구 네트워크 탭을 열어보세요. 서버에서 성공적으로 prefetch되어 클라이언트에 hydrate된 데이터이기 때문에 fetch 요청이 발생하지 않는 것을 확인할 수 있습니다. 이것이 `HydrationBoundary`의 효율성을 보여줍니다.

## SSR 비활성화하기

만약 특정 컴포넌트에서 서버 사이드 렌더링을 사용하지 않으려면, `<Suspense/>`에 `clientOnly` prop을 추가하기만 하면 됩니다. 이 경우 서버에서 데이터를 prefetch하지 않으므로 `HydrationBoundary`도 필요하지 않습니다:
...
- 서버 사이드 prefetch가 필요 없으므로 `HydrationBoundary`를 사용하지 않아도 됩니다
...
// HydrationBoundary는 내부적으로 HydrationBoundary를 사용합니다
```
...
// HydrationBoundary는 내부적으로 Hydrate를 사용합니다
```
...
| Version   | Changes                                  |
| --------- | ---------------------------------------- |
| `v3.21.0` | `<HydrationBoundary/>`가 추가되었습니다. |
