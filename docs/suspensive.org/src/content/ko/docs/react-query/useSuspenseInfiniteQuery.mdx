...
이 훅의 return 타입에는 isLoading, isError이 없습니다. `<Suspense/>`와 `<ErrorBoundary/>`가 이 훅의 data를 보장해주기 때문입니다. 또한, 이 훅의 options에는 기본적으로 suspense가 true입니다. 필요한 새 options는 @tanstack/react-query의 [useInfiniteQuery](https://tanstack.com/query/v4/docs/react/reference/useInfiniteQuery)처럼 사용하면 됩니다.
...
export const Posts = () => {
  const { data, isFetchingNextPage, isFetched, hasNextPage, fetchNextPage } = useSuspenseInfiniteQuery({
    queryKey: ['posts'],
