...
이 훅의 return 타입에는 isLoading, isError이 없습니다. `<Suspense/>` and `<ErrorBoundary/>`가 이 훅의 data를 보장해주기 때문입니다. 또한, 이 훅의 options에는 기본적으로 suspense가 true입니다. 필요한 새 options는 @tanstack/react-query의 [useQueries](https://tanstack.com/query/v4/docs/react/reference/useQueries)처럼 사용하면 됩니다.
