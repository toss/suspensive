...
Jotai에는 [tRPC](https://jotai.org/docs/extensions/trpc), [Query](https://jotai.org/docs/extensions/query), [Cache](https://jotai.org/docs/extensions/cache) 등 다양한 확장 라이브러리가 있습니다. 확장 라이브러리의 atom도 @suspensive/jotai의 `<Atom/>`, `<SetAtom/>`, `<ReadAtom/>`와 모두 호환됩니다.
...
const MyPage = () => (
  <ErrorBoundary fallback={({ error }) => <>{error.message}</>}>
    <Suspense fallback={'pending...'}>
      <ReadAtom atom={userQueryAtom}> {/* 확장 라이브러리의 atom도 호환됩니다. */}
