...
- [@suspensive/react-dom](../react-dom/installation)
- [@suspensive/jotai](../jotai/motivation)
- [@suspensive/codemods](../codemods/motivation)

### ErrorBoundary fallback내 에러 발생 시 무한 루프를 방지합니다.

@suspensive/react의 ErrorBoundary의 fallback내 에러를 처리하는 방식이 변경되었습니다.
...
따라서 @suspensive/react-query의 사용자가 @tanstack/react-query외에 다른 라이브러리를 추가로 반드시 사용하지 않도록 단순화하는 작업을 진행했습니다.
