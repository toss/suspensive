...
# !!steps When using React's lazy, you may want to preload components before they are rendered.

React's `lazy` doesn't provide a built-in way to preload components.
...
# !!steps Suspensive's lazy provides a load method for preloading components.

Just call `Component.load()` to preload the component before it's rendered.
...
# !!steps You can also handle component loading success and failure with callbacks.

Use `onSuccess` and `onError` callbacks for logging, analytics, or error handling.
...
# !!steps For version skew issues in production, use reloadOnError.

When deployments cause chunk loading failures, `reloadOnError` automatically retries by reloading the page.
