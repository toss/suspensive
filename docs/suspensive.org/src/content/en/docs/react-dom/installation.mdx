...
### Check supported react, react-dom version
...
<Table>
  <tbody>
    {[
      ...
      { library: 'react-dom', version: '^18 || ^19' },
    ].map((item) => (
...
@suspensive/react-dom lives in npm. To install the latest stable version, run the following command.
...
```shell npm2yarn
npm install @suspensive/react-dom
```
