...
<Callout type="info">
The `useForesight` hook integrates with the [`js.foresight`](https://foresightjs.com/) library to provide predictive interaction detection. This allows you to trigger callbacks before users actually click or interact with elements, enabling smart prefetching strategies.
</Callout>
...
export const Example = () => {
  ...
  const { ref, isRegistered, registerResult } = useForesight({
    callback: () => {
      setLastPrediction(`Predicted interaction at ${new Date().toLocaleTimeString()}`)
    },
    name: 'basic-example',
  })
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ marginTop: 15 }}>
        ...
        <p><strong>Touch Device:</strong> {registerResult?.isTouchDevice ? 'Yes' : 'No'}</p>
        <p><strong>Limited Connection:</strong> {registerResult?.isLimitedConnection ? 'Yes' : 'No'}</p>
        <p><strong>Last Prediction:</strong> {lastPrediction || 'None yet'}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ padding: 50 }}>
      ...
      <div style={{ marginTop: 20 }}>
        ...
        <p style={{ fontSize: 12, color: 'gray' }}>
          Move your mouse around the element to see how hitSlop affects detection.
        </p>
      </div>
    </div>
  )
...
export const Example = () => {
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ marginTop: 15 }}>
        <p>Auto Initialize: <strong>{autoInit ? 'Enabled' : 'Disabled'}</strong></p>
        <p>Registered: <strong>{isRegistered ? 'Yes' : 'No'}</strong></p>
        <p>Predictions: <strong>{predictions}</strong></p>
      </div>
    </div>
  )
    ...
    <div style={{ padding: 20 }}>
      ...
      <div style={{ marginTop: 20, padding: 15, backgroundColor: '#f5f5f5', borderRadius: 4 }}>
        ...
        <ul>
          ...
          <li><strong>Callback invoked:</strong> {callbackCount} times</li>
        </ul>
      </div>
    </div>
  )
