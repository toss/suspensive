...
`<HydrationBoundary/>` is only available when using `@tanstack/react-query` v5.
  ...
  return (
    <>
      ...
      <Suspense fallback={<div>Loading posts...</div>}>
        <HydrationBoundary queries={[postsQueryOptions(userId)]}>
...
You can also use `infiniteQueryOptions` with `HydrationBoundary`. You can mix regular queries and infinite queries in the same array. For better HTML Streaming, wrap each component with its own `Suspense` and `HydrationBoundary`:
...
const PostsPage = ({ userId }: { userId: string }) => {
  return (
    <>
      ...
      <Suspense fallback={<div>Loading posts...</div>}>
        <HydrationBoundary queries={[postsInfiniteQueryOptions(userId)]}>
...
const Page = async () => {
  return (
    <HydrationBoundary queryClient={queryClient} queries={[]}>
      {/* ... */}
    </HydrationBoundary>
  )
}
...
1. **RSC (React Server Component)**: Query execution in `HydrationBoundary`
  ...
  return (
    <>
      <Suspense fallback={<div>Loading...</div>}>
        ...
        <HydrationBoundary queries={[userQueryOptions(userId)]} timeout={1000}>
          <UserProfile />
        </HydrationBoundary>
      </Suspense>
    </>
...
With `<HydrationBoundary/>`, all of this is handled automatically:
...
When handling dependent queries, wrapping each component with a separate `HydrationBoundary` maximizes streaming benefits by creating independent Suspense boundaries. By sharing the same `queryClient`, you can use the result of the first query in subsequent queries.
...
Here's a live example demonstrating `HydrationBoundary` with Next.js streaming:
...
**Tip**: In the live demo, check the "4. no error (Best Practice)" case and open the browser's Developer Tools Network tab. You'll notice that no fetch requests are made because the data was successfully prefetched on the server and hydrated to the client, demonstrating the efficiency of `HydrationBoundary`.

## Disabling SSR

If you want to skip server-side rendering for a specific component, simply add the `clientOnly` prop to `<Suspense/>`. In this case, since you don't need to prefetch data on the server, you don't need `HydrationBoundary` either:
...
- You don't need `HydrationBoundary` since server-side prefetch is not needed
...
// HydrationBoundary internally uses HydrationBoundary
```
...
// HydrationBoundary internally uses Hydrate
```
...
| Version   | Changes                                |
| --------- | -------------------------------------- |
| `v3.21.0` | `<HydrationBoundary/>` has been added. |
