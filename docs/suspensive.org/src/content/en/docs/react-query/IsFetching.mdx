...
const GlobalFetchingIndicator = () => <IsFetching>{(isFetching) => (isFetching ? <Spinner /> : null)}</IsFetching>
```
...
const PostsFetchingIndicator = () => (
  <IsFetching queryKey={['posts']}>{(isFetching) => (isFetching ? <Spinner /> : null)}</IsFetching>
)
```
