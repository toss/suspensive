...
`<HydrateQueries/>` is an experimental feature, so this interface may change.
  ...
  return (
    <>
      ...
      <Suspense fallback={<div>Loading posts...</div>}>
        <HydrateQueries queries={[postsQueryOptions(userId)]}>
...
You can also use `infiniteQueryOptions` with `HydrateQueries`. You can mix regular queries and infinite queries in the same array. For better HTML Streaming, wrap each component with its own `Suspense` and `HydrateQueries`:
...
const PostsPage = ({ userId }: { userId: string }) => {
  return (
    <>
      ...
      <Suspense fallback={<div>Loading posts...</div>}>
        <HydrateQueries queries={[postsInfiniteQueryOptions(userId)]}>
...
const Page = async () => {
  return (
    <HydrateQueries queryClient={queryClient} queries={[]}>
      {/* ... */}
    </HydrateQueries>
  )
}
...
1. **RSC (React Server Component)**: Query execution in `HydrateQueries`
  ...
  return (
    <>
      <Suspense fallback={<div>Loading...</div>}>
        ...
        <HydrateQueries queries={[userQueryOptions(userId)]} timeout={1000}>
          <UserProfile />
        </HydrateQueries>
      </Suspense>
    </>
...
### Using HydrateQueries

With `<HydrateQueries/>`, all of this is handled automatically:
...
When handling dependent queries, wrapping each component with a separate `HydrateQueries` maximizes streaming benefits by creating independent Suspense boundaries. By sharing the same `queryClient`, you can use the result of the first query in subsequent queries.
...
Here's a live example demonstrating `HydrateQueries` with Next.js streaming:
...
**Tip**: In the live demo, check the "4. no error (Best Practice)" case and open the browser's Developer Tools Network tab. You'll notice that no fetch requests are made because the data was successfully prefetched on the server and hydrated to the client, demonstrating the efficiency of `HydrateQueries`.

## Disabling SSR

If you want to skip server-side rendering for a specific component, simply add the `clientOnly` prop to `<Suspense/>`. In this case, since you don't need to prefetch data on the server, you don't need `HydrateQueries` either:
...
- You don't need `HydrateQueries` since server-side prefetch is not needed
...
// HydrateQueries internally uses HydrationBoundary
```
...
// HydrateQueries internally uses Hydrate
```
...
| Version | Changes                                                          |
| ------- | ---------------------------------------------------------------- |
| v3.14.0 | `<HydrateQueries/>` has been added as an experimental feature. |
