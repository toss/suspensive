...
`<Hydrate/>` is only available when using `@tanstack/react-query` v4.
...
You can also use `infiniteQueryOptions` with `Hydrate`. You can mix regular queries and infinite queries in the same array. For better HTML Streaming, wrap each component with its own `Suspense` and `Hydrate`:
...
1. **RSC (React Server Component)**: Query execution in `Hydrate`
...
With `<Hydrate/>`, all of this is handled automatically:
...
When handling dependent queries, wrapping each component with a separate `Hydrate` maximizes streaming benefits by creating independent Suspense boundaries. By sharing the same `queryClient`, you can use the result of the first query in subsequent queries.
...
Here's a live example demonstrating `Hydrate` with Next.js streaming:
...
**Tip**: In the live demo, check the "4. no error (Best Practice)" case and open the browser's Developer Tools Network tab. You'll notice that no fetch requests are made because the data was successfully prefetched on the server and hydrated to the client, demonstrating the efficiency of `Hydrate`.

## Disabling SSR

If you want to skip server-side rendering for a specific component, simply add the `clientOnly` prop to `<Suspense/>`. In this case, since you don't need to prefetch data on the server, you don't need `Hydrate` either:
...
- You don't need `Hydrate` since server-side prefetch is not needed
...
// Hydrate internally uses HydrationBoundary
```
...
// Hydrate internally uses Hydrate
```
...
| Version   | Changes                      |
| --------- | ---------------------------- |
| `v3.21.0` | `<Hydrate/>` has been added. |
