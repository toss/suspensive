...
type QueriesHydrationProps = {
  ...
  timeout?: number // Optional (default: 100_000, unit: ms)
}
```
...
Limits the time allowed for fetching queries on the server. An error is thrown if query fetching exceeds this limit. The default value is 100_000ms.
