...
## Network Mode in `@suspensive/react-query` v4

Starting from Suspensive v3, the default value of `networkMode` in `@suspensive/react-query` v4 has been changed to `'always'`.  
For more details, please refer to the [Migration to v3 guide](./migrate-to-v3.mdx).
