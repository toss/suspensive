...
React gives you Suspense, lazy, and an interface to build Error Boundaries. But if you've tried to use them in a real application, you know the gaps — no built-in way to skip SSR for a Suspense boundary, implementing Error Boundaries from scratch requires boilerplate, and there's no way to coordinate any of it. Beyond React itself, data-fetching libraries and state managers have their own gaps too.
...
direction TB
A["No built-in way to skip SSR in Suspense"]
