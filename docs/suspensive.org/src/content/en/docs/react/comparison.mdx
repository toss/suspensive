...
### clientOnly — skip SSR for a Suspense boundary

React's `<Suspense>` works with SSR. When you want a boundary to render only on the client, Suspensive's `clientOnly` prop shows `fallback` on the server and `children` on the client — in one line:
