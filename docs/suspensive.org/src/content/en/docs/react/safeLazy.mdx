...
1. **First Attempt**: When a component fails to load for the first time, `safeLazy`:
...
The API is identical to `React.lazy`, so you can easily replace existing lazy components with `safeLazy` for automatic error recovery.
