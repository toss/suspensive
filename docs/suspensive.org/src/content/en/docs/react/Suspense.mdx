---
description: 'Enhanced React Suspense with clientOnly to skip SSR for a boundary, DefaultPropsProvider, and more. Drop-in replacement for React.Suspense.'
---
...
| Skip SSR for a boundary (`clientOnly`)             | ❌             | ✅                         |
| Global default fallback via `DefaultPropsProvider` | ❌             | ✅                         |
| HOC pattern (`Suspense.with`)                      | ❌             | ✅                         |

React's `<Suspense>` works with SSR. When you want a boundary to **skip server rendering** — show `fallback` on the server and render `children` only on the client — Suspensive's `clientOnly` prop declares that in one line. No `dynamic(() => import(...), { ssr: false })` or manual `useEffect` guards.
...
If you use the clientOnly prop, `<Suspense/>` returns the fallback on the server and children on the client. It does not rely on `useEffect` or mount timing — instead, it distinguishes server and client via `getServerSnapshot` and `getSnapshot` from `useSyncExternalStore`, so children are not server-side rendered.
