...
2.2.0 버전 이후로는 @tanstack/react-query의 v4와 v5을 모두 지원합니다. 최신 안정버전을 사용하고자 하면 아래 커맨드를 실행하세요.

dependencies에 @tanstack/react-query의 버전에 따라 자동으로 알맞는 @suspensive/react-query 버전을 사용하게 됩니다.
...
@tanstack/react-query v4는 @tanstack/react-query v5 보다 더 낮은 버전의 브라우저를 지원합니다.
