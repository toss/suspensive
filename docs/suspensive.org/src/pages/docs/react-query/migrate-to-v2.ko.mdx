...
### 새로운 `<SuspenseQuery/>`, `<SuspenseInfiniteQuery/>` 컴포넌트 [#775](https://github.com/suspensive/react/pull/775)

기존의 useSuspenseQuery는 훅이기 때문에 부모에 Suspense, ErrorBoundary를 배치하기 위해 Contents와 같은 이름을 가진 컴포넌트를 만들게 합니다.
이것은 Contents 내부에서 던져질 suspense와 error가 있을지 예측하기 어렵게 만듭니다.
...
const PostsPage = ({ authorId }) => (
  <ErrorBoundary fallback={({ error }) => <>{error.message}</>}>
    <Suspense fallback={'loading...'}>
      <AuthorInfo authorId={authorId} /> {/* 내부적으로 Suspense를 발생할 지 예상하기 어렵습니다. */}
      <PostList authorId={authorId} /> {/* 내부적으로 Suspense를 발생할 지 예상하기 어렵습니다. */}
    </Suspense>
  </ErrorBoundary>
)
```
...
const AuthorInfo = ({ authorId }) => {
  const { data: author } = useSuspenseQuery(userQueryOptions(authorId))
...
const PostList = ({ authorId }) => {
  const { data: selectedPosts } = useSuspenseQuery({
  ...
  <ErrorBoundary fallback={({ error }) => <>{error.message}</>}>
    <Suspense fallback={'loading...'}>
      ...
      <SuspenseQuery {...postsQueryOptions(authorId)} select={(posts) => posts.filter(({ isShow }) => isShow)}>
        {({ data: selectedPosts }) => selectedPosts.map((post) => <PostListItem {...post} />)}
      </SuspenseQuery>
    </Suspense>
  </ErrorBoundary>
)
