...
이 경우 @suspensive/react-query는 @tanstack/react-query v4와 함께 사용해 @tanstack/react-query v5의 인터페이스를 사용할 수 있게 도와주는 좋은 대안이 되어줄 수 있습니다.
...
<table>
  ...
  <tbody>
    {[
      ...
      <tr key={item.browser}>
        <td>{item.browser}</td>
        <td className="text-center">{item.version4}</td>
        <td className="text-center">{item.version5}</td>
      </tr>
    ))}
  </tbody>
</table>
