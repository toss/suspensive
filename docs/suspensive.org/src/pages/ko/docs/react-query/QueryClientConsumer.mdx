...
React hook의 제약을 피해 JSX 상에서 `useQueryClient`를 사용할 수 있게 하는 컴포넌트입니다. 컴포넌트를 통해 레거시 클래스 컴포넌트나 특정 환경(SSR, 마이크로 프론트엔드)에서 선언적으로 QueryClient에 접근할 수 있도록 도와줍니다.

사용자 환경에 설치되어 있는 `@tanstack/react-query`의 버전에 따라 props 인터페이스가 다릅니다. `@tanstack/react-query` v4 버전이 설치되어 있다면 `props.context`를 사용하고 v5버전 이면 `props.queryClient`를 사용하게 됩니다.
...
`props.queryClient`에 값을 전달하지 않는다면, 컴포넌트와 가장 가까운 QueryClient를 사용하게 됩니다.
...
const PostsPage = () => {
  ...
  return (
    <>
      {/* @tanstack/react-query v5을 사용하신다면 props.queryClinet를 사용할 수 있습니다. */}
...
`props.context`에 값을 전달하지 않는다면, 컴포넌트와 가장 가까운 QueryClient를 사용하게 됩니다.
...
const PostsPage = () => {
  ...
  return (
    <>
      {/* @tanstack/react-query v4을 사용하신다면 props.context를 사용할 수 있습니다. */}
