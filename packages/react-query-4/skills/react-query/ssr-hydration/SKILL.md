...
name: ssr-hydration
description: >-
  Server rendering and hydration for TanStack Query v4 in RSC frameworks:
  ...
  per-boundary HTML streaming, QueryClientConsumer. Load when setting up
  QueryClient in Next.js App Router, prefetching queries in Server Components,
  or debugging cross-request cache leaks and hydration issues.
metadata:
  type: sub-skill
...
Two experimental APIs cover queries in SSR: `createGetQueryClient` manages the QueryClient lifecycle (new instance per server request, singleton in the browser), and `QueriesHydration` — an async Server Component — prefetches an array of queries on the server and hydrates them into client components, replacing manual QueryClient + prefetchQuery + dehydrate + Hydrate boilerplate (TSQ v4's Hydrate is what v5 renamed to HydrationBoundary; QueriesHydration wraps it internally). Both interfaces are experimental and may change.

On the server, `getQueryClient` forces the cache removal time (`cacheTime` in v4 — renamed `gcTime` in v5) to `Infinity` — automatic and non-overridable — so caches survive until the request completes and no `setTimeout` GC scheduling piles up under concurrent requests. The browser instance uses your configured value.
...
### QueryClientConsumer: read the QueryClient in JSX (client components)

`QueryClientConsumer` (experimental) exposes useQueryClient as a render prop for list rows and conditionals. On v4 it takes an optional `context` prop — TSQ v4's custom QueryClient context — where v5 takes a `queryClient` prop instead (the codemod `npx @suspensive/codemods migrate-query-client-consumer-props .` renames it when you upgrade).

```tsx
'use client'

import { QueryClientConsumer } from '@suspensive/react-query-4'
