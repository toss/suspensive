 ...
 *
 * @description
 * HydrationBoundary is designed for React Server Components (RSC).
 ...
 *         <HydrationBoundary queries={[userQueryOptions(userId)]}>
 *           <UserProfile />
 *         </HydrationBoundary>
 *       </Suspense>
 *
 *       <Suspense fallback={<div>Loading posts...</div>}>
 *         <HydrationBoundary queries={[postsQueryOptions()]}>
 *           <PostsList />
 *         </HydrationBoundary>
 *       </Suspense>
 *     </>
 ...
 * ```
 *
 * @see {@link https://suspensive.org/docs/react-query/HydrationBoundary Documentation}
 */
export async function HydrationBoundary({
  queries,
  children,
  ...
  return (
    <TanstackHydrationBoundary {...props} state={dehydrate(queryClient)}>
      {children}
...
const createTimeoutController = (ms: number) => {
  ...
  return {
    promise: new Promise<never>((_, reject) => {
      timerId = setTimeout(() => reject(new Error(`HydrationBoundary: timeout after ${ms} ms`)), ms)
    }),
