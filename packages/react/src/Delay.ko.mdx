...
이 컴포넌트는 children이 노출되는 시점을 ms만큼 지연시킵니다.
...
위와 같은 코드에서는 200ms만큼 children 노출이 지연됩니다.
...
`<Delay/>`는 `<SuspensiveProvider/>`와 함께 사용할 때 더욱 강력해집니다.
`<SuspensiveProvider/>`를 사용하여 여러 `<Delay/>` default ms를 제어합니다.
